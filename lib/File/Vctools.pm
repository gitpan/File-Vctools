...
projects. Currently, File::Vctools only runs on Windows and Linux, but if somebody
...
Here is a first program that runs the components of File::Vctools:
  ...
  mkdir File::Spec->catdir($tempdir, 'Data');
  mkdir File::Spec->catdir($tempdir, 'Project');
  ...
  write_file(File::Spec->catfile($tempdir, 'Data', 'file.txt'),
    qq{Line001\n},
    qq{Line002\n},
    qq{Line003\n},
    qq{Line004\n},
  ...
  chdir File::Spec->catdir($tempdir, 'Project') or die "Error chdir $!";

  system $^X, File::Spec->catfile($mpath, 'vc_init.pl');
  system $^X, File::Spec->catfile($mpath, 'vc_checkout.pl'), File::Spec->catfile($tempdir, 'Data', 'file.txt');
...
Here is the output (on a Windows system):
  ...
  # ****************************
  #
  # Project: ~\AppData\Local\Temp\6ALx6IvP22\Project
  #
  # Item [dir]  ==> Work                      : *** Write ***
  # Item [dir]  ==> Data                      : *** Write ***
  # Item [dir]  ==> Cmd                       : *** Write ***
  ...
  #
  #    1. [CO=  1] Project         ~\AppData\Local\Temp\6ALx6IvP22\Data\file.txt
  #
...
up a subdirectory 'Work', a directory 'Data' and a directory 'Cmd' with 9 "shortcut" programs ('r_*.pl'):
...
Suppose you have a file /home/user1/Documents/Book/oscar_wilde.txt with some text in
Ascii format:

  ~/Projects/NewProject> cat /home/user1/Documents/Book/oscar_wilde.txt
...
as follows:
  ...
  ~/Projects/NewProject> perl /opt/perl/site/bin/vc_checkout.pl /home/user1/Documents/Book/oscar_wilde.txt
  ...
  # Project: ~/Projects/NewProject
  #
  # Ckout [  1/  1] ** Write ** F_oscar_wilde_Z001.txt         << ~/Documents/Book/oscar_wilde.txt

A new file ('F_oscar_wilde_Z001.txt') has been created in the subdirectory Work/ (don't worry about the
'B_Flist.xml' file, we will discuss this file later)
  ...
  # total 2
  # -rw-r--r-- 1 user1 user1 120 2010-04-09 17:21 B_Flist.xml
  # -rw-r--r-- 1 user1 user1 538 2010-04-09 17:31 F_oscar_wilde_Z001.txt
...
file 'F_oscar_wilde_Z001.txt' using your favourite ASCII editor (vi, emacs, notepad,
UltraEdit, SciTE, ...). Let's say you change the word 'sapphires' into 'diamonds', then you
save the file.

  ~/Projects/NewProject/Work> scite F_oscar_wilde_Z001.txt
...
as follows:
  ...
  # Apply [  1/  1] F_oscar_wilde_Z001.txt                                WRK --I=0001/D=0001--> ORG

  ~/Projects/NewProject> 

To confirm that your changes ('...bright diamonds...') have made it correctly into the both files
('F_oscar_wilde_Z001.txt' and 'oscar_wilde.txt'), we issue the following commands:

  ~/Projects/NewProject> grep "bright" Work/F_oscar_wilde_Z001.txt

  # he had two bright diamonds, and a large red ruby glowed on his sword-hilt.

  ~/Projects/NewProject> grep "bright" /home/user1/Documents/Book/oscar_wilde.txt
...
To review your changes, simply call vc_status.pl with options -a -o -e:
  ...
  #
  # ===============================================================================================
  # [  1/  1] F_oscar_wilde_Z001.txt         ARC --I=0001/D=0001--> WRK -----------------> ORG
  # ===============================================================================================
  # ARC: 06/04/2010 16:37:51 ~/Programs/vc_archive/F_=home=user1=Documents=Book=oscar_wilde.txt
  # WRK: 09/04/2010 17:35:59 ~/Projects/NewProject/Work/F_oscar_wilde_Z001.txt
  # ORG: 09/04/2010 17:36:06 ~/Documents/Book/oscar_wilde.txt
  #
  # ARC->WRK: 3c3
  # ARC->WRK: < he had two bright sapphires, and a large red ruby glowed on his sword-hilt.
  # ARC->WRK: ---
  # ARC->WRK: > he had two bright diamonds, and a large red ruby glowed on his sword-hilt.

  ~/Projects/NewProject>

If you prefer a different format, you replace the option '-e' by '-u':
...
You can choose to back out the changes you made in the original file /home/user1/Documents/Book/oscar_wilde.txt,
and, at the same time, still keep the changes in /home/user1/Projects/NewProject/Work/F_oscar_wilde_Z001.txt (in
case you want to come back to those changes later). The command is as follows:
  ...
  # Reset [  1/  3] rewrite ==> ~/Documents/Book/oscar_wilde.txt

  ~/Projects/NewProject>

To confirm that your changes ('...bright diamonds...') are still in ('F_oscar_wilde_Z001.txt'), but *not*
in ('oscar_wilde.txt') -- that file should now have the original '...bright sapphires...' -- we issue the
following commands:
...
If we want to get an overview of all archived files, we simply issue the vc_list command:
  ...
  #
  #    1. [CO=  1]                 ~/Documents/Book/oscar_wilde.txt
...
To get the details of which projects the files actually belong to, we add the option '-d':
  ...
  #
  #    1. [CO=  1]                 ~/Documents/Book/oscar_wilde.txt
  #     +  NewProject      ----> F_oscar_wilde_Z001.txt         09-Apr-2010 17:31:30 ~/Projects

  ~>

The '+' character (in front of the 'NewProject') indicates that the file has been modified, but not
yet applied to ~/Documents/Book/oscar_wilde.txt (that's because we have previously run vc_reset.pl)
...
the old '+'):
  ...
  # Reading ~/Programs/vc_archive/D_Coutlist.dat
  #
  #    1. [CO=  1] NewProject      ~/Documents/Book/oscar_wilde.txt
  #     => NewProject      ----> F_oscar_wilde_Z001.txt         09-Apr-2010 17:31:30 ~/Projects
...
file ('oscar_wilde.txt'), but on a different project, and this person is sending you now his
...
modification of the word '...fine gold...' into '...pure gold...':

  ~/Projects/NewProject> cat patch.txt

  # --- a/oscar_wilde.txt  2010-04-06 14:25:07.000000000 +0200
  # +++ b/oscar_wilde.txt  2010-04-06 14:27:51.000000000 +0200
  # @@ -2,4 +2,4 @@
  #  High above the city, on a tall column, stood the statue of the Happy
  # -Prince. He was gilded all over with thin leaves of fine gold, for eyes
  # +Prince. He was gilded all over with thin leaves of pure gold, for eyes
  #  he had two bright sapphires, and a large red ruby glowed on his sword-hilt.
...
be issued:
  ...
  ~/Projects/NewProject> perl /opt/perl/site/bin/vc_merge.pl
     --input=Work/F_oscar_wilde_Z001.txt --diff=patch.txt --output=out.txt

  # *******************************
  # ** Merging diffs into a file **
  # *******************************
  #
  # input file  = 'Work/F_oscar_wilde_Z001.txt'
  # diff        = 'patch.txt'
  # output file = 'out.txt'
  #
  # There are 1 hunks:
  #   1. @@ -00002,00004 +00002,00004 @@ --> High above the city, on a...wed on his sword-hilt.
  # 
  # Error-0040: Conflict in hunk 1 (@@ -2,4 +2,4 @@): 0 matches at /opt/perl/site/bin/vc_merge.pl line 131.

  ~/Projects/NewProject>

We notice that there is a conflict between his changes and our changes. To resolve this conflict, we have to
investigate the details of what constitutes the changes. If we re-run vc_merge.pl, but we replace the
'--output=out.txt' parameter by '--linewd=75', then we get a detailed report (alignment) of the merge:

  ~/Projects/NewProject> perl /opt/perl/site/bin/vc_merge.pl
     --input=Work/F_oscar_wilde_Z001.txt --diff=patch.txt --linewd=75
  ...
  #
  # ========================================
  # ==>   1. @@ -00002,00004 +00002,00004 @@
  # ========================================
  #
  #    s    2 => Princ     e.`H e`was`gilded`   a   ll` over  `with  `thin`le   av es`of`fin
  #    ****** => ** ******* * **  ***** **** *** ***   * ***** ** ***   **** *** ** *    ***
  #    d    1 => H igh`above `the`c    ity, `on`a`tall`column,`s tood`th    e`statue `of`th 
  #
  #              e`gold,`for`eyes
  #                *********** **
  #              e`Happ       y  
  #
  #    s    3 => h    e `h   a     d  `two`b  r  ight `d iamonds,`and`a`l arge `r     ed`rub
  #    ****** => ***** * **** ***** ** *** *** ** ** * ** *** *** ****** * ** * ****** * ***
  #    d    2 => Prince.`He`was`gilded`all`over`wi  th`thi   n   `      leav es`of`fine `   
  #
  #              y`glowed ` on`hi s`sword-hilt.
  #              ** * ** * * * *** ************
  #                g ol d,`for`eyes            
  #
  #    s    4 =>                                                                            
  #    ****** => ***************************************************************************
  #    d    3 => he`had`two`bright`sapphires,`and`a`large`red`ruby`glowed`on`his`sword-hilt.

  ~/Projects/NewProject> 

Unfortunately, this report doesn't help us very much. In fact, it seems that the alignment of the lines
is incorrect. If you look closely, you will notice that the "@@ -00002,00004 +00002,00004 @@" in the
patch file seems to be wrong, if it was "@@ -00001,00003 +00001,00003 @@", the report would align much
better.

So we decide to manipulate the 'patch.txt' file and replace '@@ -2,4 +2,4 @@' by '@@ -1,3 +1,3 @@', as
follows:
  ...
  # --- a/oscar_wilde.txt  2010-04-06 16:00:03.000000000 +0200
  # +++ b/oscar_wilde.txt  2010-04-06 16:00:03.000000000 +0200
  # @@ -1,3 +1,3 @@
  #  High above the city, on a tall column, stood the statue of the Happy
  # -Prince. He was gilded all over with thin leaves of fine gold, for eyes
  # +Prince. He was gilded all over with thin leaves of pure gold, for eyes
...
Now we re-run exactly the same 'vc_merge.pl ... --linewd=75', and we get:
  ...
  #
  # ========================================
  # ==>   1. @@ -00001,00003 +00001,00003 @@
  # ========================================
  #
  #    s    1 => High`above`the`city,`on`a`tall`column,`stood`the`statue`of`the`Happy
  #    -   -- =>                                                                     
  #    d    1 => High`above`the`city,`on`a`tall`column,`stood`the`statue`of`the`Happy
  #
  #    s    2 => Prince.`He`was`gilded`all`over`with`thin`leaves`of`fine`gold,`for`eyes
  #    -   -- =>                                                                       
  #    d    2 => Prince.`He`was`gilded`all`over`with`thin`leaves`of`fine`gold,`for`eyes
  #
  #    s    3 => he`had`two`bright`diamond  s,`and`a`large`red`ruby`glowed`on`his`sword-hilt
  #    ****** =>                   ** ******                                                
  #    d    3 => he`had`two`bright`s apphires,`and`a`large`red`ruby`glowed`on`his`sword-hilt

  ~/Projects/NewProject>

That's much better now, we can easily identify that indeed, there is a conflict, and the
conflict is caused by the modification of the word 'sapphires' into 'diamonds'.

To resolve the conflict, we decide to intervene manually in the 'patch.txt' file and change
the word 'sapphires' into 'diamonds':
...
We come back to our initial command 'vc_merge.pl ... --output=out.txt', which now succeeds:
  ...
  #
  # There are 1 hunks:
  #   1. @@ -00001,00003 +00001,00003 @@ --> High above the city, on a...wed on his sword-hilt.
  #
  # Output successfully written to 'out.txt'

  ~/Projects/NewProject>

We confirm that both changes (first change was '...sapphires...' into '...diamonds...', second change
was '...fine...' into '...pure...') are in the file 'out.txt':
...
When you run 'vc_init.pl', the following 9 "shortcut" programs are automatically created in
directory 'Cmd': r_apply.pl, r_checkout.pl, r_list_det.pl, r_list_file.pl, r_list_proj.pl,
r_renew.pl, r_reset.pl, r_statdiff.pl and r_status.pl.
...
than one book in ~/Documents/Book/
  ...
  # -rw-r--r-- 1 user1 user1 538 2010-04-06 16:37 oscar_wilde.txt
...
'oscar_wilde.txt', as well as a new second file, 'jules_verne.txt'):
  ...
  # <?xml version="1.0" encoding="iso-8859-1"?>
  # <checkout>
  #   <file name="/home/user1/Documents/Book/oscar_wilde.txt" />
...
file 'oscar_wilde.txt', as well as another file, 'lewis_carroll.txt'):
...
home directory):
  ...
  # Project: ~/Projects/NewProject
  #
  # Ckout [  1/  2]             F_oscar_wilde_Z001.txt         -- ~/Documents/Book/oscar_wilde.txt
  ...
  # Project: ~/Projects/OtherProject
  #
  # Ckout [  1/  2] ** Write ** F_oscar_wilde_Z001.txt         << ~/Documents/Book/oscar_wilde.txt
  # Ckout [  2/  2] ** Write ** F_lewis_carroll_Z001.txt       << ~/Documents/Book/lewis_carroll.txt

  ~>

Please note that the checkout of 'NewProject' did not write 'F_oscar_wilde_Z001.txt'.
That's completely normal, because we already had checked out 'oscar_wilde.txt' to 'NewProject' previously
(with modification of the word 'sapphires' into 'diamonds'). It is also important to notice that the
checkout of 'oscar_wilde.txt' for project 'OtherProject' does not contain that modification.

=head2 r_apply.pl

To make changes in '~/Documents/Book/' (files 'oscar_wilde.txt', 'jules_verne.txt'
...
We will use 'r_apply.pl' later to transfer the updated file into '~/Documents/Book/'.

In 'F_oscar_wilde_Z001.txt' ('OtherProject'), we change "bright sapphires" into "bright gemstones":

  ~> grep "bright" Projects/OtherProject/Work/F_oscar_wilde_Z001.txt

  # he had two bright sapphires, and a large red ruby glowed on his sword-hilt.

  ~> scite Projects/OtherProject/Work/F_oscar_wilde_Z001.txt

  ~> grep "bright" Projects/OtherProject/Work/F_oscar_wilde_Z001.txt

  # he had two bright gemstones, and a large red ruby glowed on his sword-hilt.

  ~> 

In 'F_lewis_carroll_Z001.txt' ('OtherProject'), we change "picking the daisies" into "picking the flowers":
...
In 'F_jules_verne_Z001.txt' ('NewProject'), we change "People said" into "People thought":
...
is no need to change directory, we stay in our home directory):
  ...
  # Apply [  2/  2] F_oscar_wilde_Z001.txt                                WRK ===== Update ====> ORG
...
One last thing to notice here is that the second 'r_apply.pl' (with the 'oscar_wilde.txt' that
contained the text '...bright gemstones...') overwrote the first 'r_apply.pl' (with the
'oscar_wilde.txt' that contained the text '...bright diamonds...'). In fact we find '...bright
gemstones...' in the file '~/Documents/Book/oscar_wilde.txt':

  ~> grep "bright" Documents/Book/oscar_wilde.txt
...
Here is the status for 'OtherProject':
  ...
  #
  # ===============================================================================================
  # [  2/  2] F_oscar_wilde_Z001.txt         ARC --I=0001/D=0001--> WRK -----------------> ORG
  # ===============================================================================================
  # ARC: 10/04/2010 14:43:24 ~/Programs/vc_archive/F_=home=klaus=Documents=Book=oscar_wilde.txt
  # WRK: 10/04/2010 15:28:21 ~/Projects/OtherProject/Work/F_oscar_wilde_Z001.txt
  # ORG: 10/04/2010 15:48:31 ~/Documents/Book/oscar_wilde.txt
  #
  # ARC->WRK: 3c3
  # ARC->WRK: < he had two bright sapphires, and a large red ruby glowed on his sword-hilt.
  # ARC->WRK: ---
  # ARC->WRK: > he had two bright gemstones, and a large red ruby glowed on his sword-hilt.

=head2 r_statdiff.pl

The command 'r_statdiff.pl' provides the same information in a different format:
...
therefore we have to pick one -- We choose 'OtherProject'):
  ...
  #    3. [CO=  2] OtherProject    ~/Documents/Book/oscar_wilde.txt
...
To get information about all files in the project, we run the 'r_list_file.pl' command:
  ...
  # List  [  3/  4] F_oscar_wilde_Z001.txt         +  NewProject                ~/Projects
  # List  [  4/  4] F_oscar_wilde_Z001.txt         => OtherProject              ~/Projects
...
for 'oscar_wilde.txt' in project 'NewProject')

=head2 r_list_det.pl

The same information, but in a different format can be displayed with the 'r_list-det.pl' command:
  ...
  #
  #    3. [CO=  2] OtherProject    ~/Documents/Book/oscar_wilde.txt
  #     +  NewProject      ----> F_oscar_wilde_Z001.txt         10-Apr-2010 14:43:24 ~/Projects
  #     => OtherProject    ----> F_oscar_wilde_Z001.txt         10-Apr-2010 14:45:46 ~/Projects
...
In order to reset all apply-commands, the 'r_reset.pl' command can be used:
  ...
  # Reset [  3/  3] rewrite ==> ~/Documents/Book/oscar_wilde.txt
...
If we re-run the 'r_list_det.pl' command, we will see that the '=>' indicators have changed into '+':
  ...
  #     +  OtherProject    ----> F_lewis_carroll_Z001.txt       10-Apr-2010 14:45:46 ~/Projects
  #
  #    3. [CO=  2]                 ~/Documents/Book/oscar_wilde.txt
  #     +  NewProject      ----> F_oscar_wilde_Z001.txt         10-Apr-2010 14:43:24 ~/Projects
  #     +  OtherProject    ----> F_oscar_wilde_Z001.txt         10-Apr-2010 14:45:46 ~/Projects
...
performs the following commands: 'vc_reset.pl', 'vc_list.pl', 'vc_checkout.pl' and 'vc_apply.pl'.
  ...
  # Reset [  3/  3]         ==> ~/Documents/Book/oscar_wilde.txt
  # List  [  1/  4] F_jules_verne_Z001.txt         +  NewProject                ~/Projects
  # List  [  2/  4] F_lewis_carroll_Z001.txt       +  OtherProject              ~/Projects
  # List  [  3/  4] F_oscar_wilde_Z001.txt         +  NewProject                ~/Projects
  # List  [  4/  4] F_oscar_wilde_Z001.txt         +  OtherProject              ~/Projects
